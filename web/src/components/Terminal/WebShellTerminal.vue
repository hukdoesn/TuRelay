<template>
    <a-layout>
            ...
            <a-layout>
                <a-layout-content class="terminal-content">
                    ...
                    <div class="tab-bar">
                        ...
                        <!-- 按钮容器，包含文件管理和重新连接按钮 -->
                        <div class="button-container" v-if="currentHostId">
                            <!-- 文件管理按钮 -->
        ...
        <!-- 文件管理抽屉 -->
        <a-drawer v-model:open="isFileManagerVisible" title="文件管理" :width="900" :destroyOnClose="true" @close="handleDrawerClose">
            <!-- 文件列表和上传下载功能 -->
            <div class="file-manager-content">
                ...
                <div class="path-and-actions">
                    ...
                    <!-- 下载按钮 -->
                    <a-button @click="downloadSelectedFiles"
                        :disabled="selectedFileKeys.length === 0"
                        class="action-button spacing">
                        下载
                    </a-button>
                    <!-- 删除按钮 -->
                    <a-button @click="deleteSelectedFiles"
                        :disabled="selectedFileKeys.length === 0"
                        class="action-button spacing">
                        <DeleteOutlined />
                        删除
                ...
                <!-- 文件列表 -->
                <a-table :columns="fileColumns" :dataSource="fileList" :rowKey="record => record.filename"
                    :pagination="false" :scroll="{ y: 700 }" :rowSelection="rowSelection">
                </a-table>
            </div>
        </a-drawer>
    </a-layout>
...
const expandAndSelectNode = (hostId) => {
    ...
    if (result) {
        ...
        addTabWithUniqueName(node.title, node.key);
    } else {
...
const removeTab = (tab) => {
    ...
    // 如果删除的tab是当前活动的tab，需要更新activeTabKey和currentHostId
...
const handleFileClick = async (record) => {
    if (record.is_directory) {
        // 如果是目录，进入该目录
        const newPath = currentPath.value.endsWith('/') ? currentPath.value + record.filename : currentPath.value + '/' + record.filename;
...
.action-button {
    ...
    display: flex;
    align-items: center;
    border: 1px solid transparent !important; /* 添加透明边框*/
...
.ant-table {
    background-color: #141414 !important; /* Table background color */
    color: #ffffffD9 !important;           /* Table text color */
}

/* Change the background color and text color of the table header */
.ant-table-thead > tr > th {
    background-color: #141414 !important; /* Header background color */
    color: #ffffffD9 !important;          /* Header text color */
    border-top: none !important;               /* Remove all borders */
    border-bottom: none !important;  
    border-bottom: 1px solid #303030 !important; /* Dividing line below header */
}
...
.ant-table-tbody > tr > td {
    background-color: #141414 !important; /* Row background color */
    color: #ffffffD9 !important;          /* Row text color */
}

/* Change the background color of a row when the mouse hovers over it to #424242 */
.ant-table-tbody > tr:hover > td {
    background-color: #1f1f1f !important; /* Row hover background color */
}

/* Set the dividing line (border) between table rows to #424242 */
.ant-table-tbody > tr > td {
    border: none !important; /* Remove all borders */
    border-bottom: 1px solid #303030 !important; /* Row dividing line color */
}
...
.ant-checkbox-inner {
    background-color: #141414 !important; /* Checkbox background when unchecked */
    border-color: #424242 !important;     /* Checkbox border when unchecked */
}

/* Checked checkbox state */
.ant-checkbox-checked .ant-checkbox-inner {
    background-color: #424242 !important; /* Checkbox background when checked */
    border-color: #424242 !important;     /* Checkbox border when checked */
}

/* Change the color of the checkmark inside the checkbox */
.ant-checkbox-checked .ant-checkbox-inner::after {
    border-color: #ffffffD9 !important;   /* Checkmark color */
}

/* Checkbox hover state */
.ant-checkbox:hover .ant-checkbox-inner {
    border-color: #1f1f1f !important;     /* Checkbox border color on hover */
}

/* Checkbox focus state */
.ant-checkbox-input:focus + .ant-checkbox-inner {
    border-color: #1f1f1f !important;     /* Checkbox border color when focused */
}
